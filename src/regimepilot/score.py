...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    Usage: ``python -m regimepilot.score --csv path/to/bars.csv [--json]``
    """
    tolerant_console()
    ...
    if "--csv" not in arguments or arguments.index("--csv") + 1 >= len(arguments):
        print("usage: python -m regimepilot.score --csv path/to/bars.csv [--json]", file=sys.stderr)
        return 1
    ...
    card = compute_scorecard(trades, daily_bars_from_minute_bars(bars))
