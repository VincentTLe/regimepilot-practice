...
import positions
from models import LegPosition, LegQuote, OpenSpread
...
def test_parse_occ_round_trip():
    assert positions.parse_occ("SPY260911C00650000") == ("SPY", EXP, "C", 650.0)
    assert positions.parse_occ("IWM260911P00230500") == ("IWM", EXP, "P", 230.5)


@pytest.mark.parametrize("bad", ["", "SPY", "SPY260911X00650000", "SPY261341C00650000", "spy 650 call"])
def test_parse_occ_rejects_garbage(bad):
    assert positions.parse_occ(bad) is None
...
def test_pair_spreads_happy_path():
    ...
    spreads, warnings = positions.pair_spreads(legs)
    assert warnings == []
...
def test_pair_spreads_warns_and_never_touches_odd_shapes(legs):
    spreads, warnings = positions.pair_spreads(tuple(legs))
    assert spreads == []
...
def test_pair_spreads_non_debit_pair_has_unknown_debit():
    ...
    spreads, _ = positions.pair_spreads(legs)
...
def test_exit_at_exact_stop_threshold():
    ...
    decision = positions.exit_decision(spread(), quote(1.4, 1.6), quote(0.4, 0.6), TODAY)
...
def test_missing_marks_or_unknown_debit_hold_instead_of_guessing():
    assert positions.exit_decision(spread(), None, quote(1, 1.2), TODAY) is None
    assert positions.exit_decision(spread(), quote(1, 1.2), quote(None, None), TODAY) is None
    assert positions.exit_decision(spread(debit=None), quote(0.1, 0.2), quote(0.0, 0.1), TODAY) is None
...
def test_open_premium_at_risk_sums_or_refuses():
    ...
    assert positions.open_premium_at_risk(known) == 300.0
    assert positions.open_premium_at_risk([spread(debit=None)]) is None


def test_size_entry_caps_on_100k_equity():
    # per-entry cap 0.5% of 100k = $500; debit $2.00 -> $200/contract -> qty 2
    qty, reason = positions.size_entry(2.0, 100_000.0, 0.0, 0.0)
    assert (qty, reason) == (2, None)


def test_size_entry_refusals():
    assert positions.size_entry(2.0, None, 0.0, 0.0) == (0, "unknown_equity")
    assert positions.size_entry(2.0, 0.0, 0.0, 0.0) == (0, "unknown_equity")
    assert positions.size_entry(2.0, 100_000.0, None, 0.0) == (0, "unknown_open_risk")
    assert positions.size_entry(0.0, 100_000.0, 0.0, 0.0) == (0, "bad_debit")
    assert positions.size_entry(6.0, 100_000.0, 0.0, 0.0) == (0, "risk_caps")  # $600 > $500


def test_size_entry_cycle_and_total_room():
    # cycle cap 1% = $1000; already spent $900 -> only $100 left -> qty 0 at $2 debit
    assert positions.size_entry(2.0, 100_000.0, 0.0, 900.0) == (0, "risk_caps")
    # total cap 10% = $10k; open risk $9,900 -> $100 room -> refused
    assert positions.size_entry(2.0, 100_000.0, 9_900.0, 0.0) == (0, "risk_caps")
    # open risk exactly at cap -> refused
    assert positions.size_entry(2.0, 100_000.0, 10_000.0, 0.0) == (0, "risk_caps")
